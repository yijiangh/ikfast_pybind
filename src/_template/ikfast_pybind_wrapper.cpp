...
#include <[put_extension]/ikfast.h>
...
PYBIND11_MODULE(ikfast_[put_extension], m)
{
    m.doc() = R"pbdoc(
        [put_extension]
        -----------------------
        .. currentmodule:: [put_extension]
        .. autosummary::
           :toctree: _generate
           get_ik
           get_fk
    ...
    py::arg("free_jt_vals"),
    R"pbdoc(
        get inverse kinematic solutions for [put_extension]
    )pbdoc");
    ...
    py::arg("joint_list"),
    R"pbdoc(
        get forward kinematic solutions for the [put_extension] robot
    )pbdoc");
